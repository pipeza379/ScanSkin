...
class Product extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return ((this.props.getSkinType !== nextProps.getSkinType) || (this.state.products_filter !== nextState.products_filter))
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.getSkinType !== nextProps.getSkinType) {
      ...
      let products_filter = this.filterSkin(select_skin_type, this.state.products)
      console.log(products_filter)
      this.setState({
        products_filter,
      })
    ...
    brands.forEach(brand => {
      ...
      key.forEach(id => {
        for (let i = 0; i < sel.length; i++) {
          ...
          else if ((type !== "") && (type === normal || type === dry || type === oily || type === sensitive)) {
            products_brand_filter[id] = products[brand][id]
            break
          }
        }
